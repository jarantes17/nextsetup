...
function renderCSS(props: any, currentBreakpoint: string) {
  if (!props) return ''
...
export function Image({ as, ...props }: any): JSX.Element {
  const tag = 'img'
...
export function Input({ as, styleSheet, ...props }: any): JSX.Element {
  const tag = 'input'
...
export function Button({ as, styleSheet, ...props }: any): JSX.Element {
  ...
  const finalStyleSheet = {
    ...
    border: `${theme.space.xpx} solid ${
      (theme.colors as any)[buttonVariant][900]
    }`,
    borderRadius: theme.space.x2,
    paddingHorizontal: {
      xs: theme.space.x5,
    ...
    paddingVertical: theme.space.x3,
    transition: 'all 0.2s ease-in-out',
    backgroundColor: (theme.colors as any)[buttonVariant][600],
    hover: {
      backgroundColor: (theme.colors as any)[buttonVariant][500],
      boxShadow: `0 5px 10px -5px ${theme.colors.neutral[999]}73`
    },
    focus: {
      backgroundColor: (theme.colors as any)[buttonVariant][700],
